...
class NoticiasRepository extends EntityRepository
{
	...
	public function findNoticiasNuevasAlumno($alumno,$idResponsable, $categoria)
	{
		return $this->getEntityManager()->createQuery(
			'SELECT n.id FROM ColeBundle:Noticias n WHERE n.categoria=:categoria AND 
			n.id IN(select a.idAviso FROM IntranetBundle:Avisos a where a.idUsuario=:alumno AND a.idResponsable=:idResponsable AND a.tipoUsuario=:tipoUsuario  AND a.tipoAviso=:tipoAviso) ORDER BY n.fecha DESC')
		->setParameters(array(
			'alumno' => $alumno,
